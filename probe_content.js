...
(async () => {
  ...
  const MF = await page.evaluate(({ пуб }) => {
    const п = (o) => Object.assign({}, пуб, o || {});
    const отч = (id, pid, назв, период, реелсы) => ({ id: id, project_id: pid, title: назв,
      published_at: '2026-08-0' + id + 'T00:00:00Z',
      payload: { period: период, posts: [], reels: реелсы.map(t => п({ title: t })) } });
    PROJECTS.length = 0;
    PROJECTS.push({ id: 'p1', name: 'Artel' }, { id: 'p2', name: 'APOLO' },
                  { id: 'p3', name: 'Третий' });
    CT_RAW = [
      отч('1', 'p1', 'SMM', '', ['раз', 'два']),                 // Artel — без периода
    ...
    /* Фильтр формата отсёк всё — тут кнопка сброса обязана быть. */
    CT.project = 'p1'; CT.period = ''; CT.kind = 'post'; renderContentFx();
    const отсеклоФорматом = {
      заголовок: (document.querySelector('#content-ag .cfx-empty b')||{}).textContent,
      кнопка: !!document.querySelector('#content-ag .cfx-reset') };
    const до = ctState().units.length;
    const кн = document.querySelector('#content-ag .cfx-reset');
    if(кн) кн.click();
    const послеСброса = { публикаций: ctState().units.length, формат: CT.kind, профиль: CT.project, до: до };
    ...
    return { общие, уArtel, уApolo, тупик, пустойОтчёт, отсеклоФорматом, послеСброса, совсемПусто };
  }, { пуб: пуб() });
  ...
  ok('у профиля без периодов списка периодов нет вовсе',
      MF.уArtel.списков === 2 && MF.уArtel.пункты === null, MF.уArtel);
  ...
  ok('невозможная пара «профиль + период» чинится сама',
      MF.тупик.период === '' && MF.тупик.публикаций === 2 && !MF.тупик.пусто, MF.тупик);
  ...
  ok('отсечённое фильтром названо фильтром',
      /фильтры/.test(MF.отсеклоФорматом.заголовок || '') && MF.отсеклоФорматом.кнопка, MF.отсеклоФорматом);
  ok('кнопка «Показать всё» возвращает данные на экран',
      MF.послеСброса.до === 0 && MF.послеСброса.публикаций === 4
      && MF.послеСброса.формат === 'all' && MF.послеСброса.профиль === '', MF.послеСброса);
