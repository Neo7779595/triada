...
const ГАЙДЫ = [
  ['Академия · Старт',            'Старт — первые 30 дней',        'https://detroyd.com/guides/start.html'],
...
(async () => {
  ...
  const D = await page.evaluate((гайды) => {
    ...
    const res = {
      ...
      /* Плитку добавления берём у категории с заведомо ДЛИННЫМ именем: у
         короткого подпись остаётся полной, и проверка «не разъезжается»
         прошла бы мимо своего смысла. */
      длиннаяКатегория: (cats.find(c => c.cat.length > 16) || {}).cat,
      подписьДобавить: (function(){ const i = cats.findIndex(c => c.cat.length > 16);
        return i < 0 ? null : ((document.querySelectorAll('#tools-body .tool-add')[i] || {}).textContent || null); })(),
      подсказкаДобавить: (function(){ const i = cats.findIndex(c => c.cat.length > 16);
        return i < 0 ? null : ((document.querySelectorAll('#tools-body .tool-add')[i] || {}).title || null); })(),
    };
  ...
  /* Первый материал берём из самой описи: порядок академии меняется, когда
     добавляют разбор, и вбитое имя устарело бы на первой же правке. */
  ok('в подписи полное название', D.имена[0] === ГАЙДЫ[0][1], [D.имена[0], ГАЙДЫ[0][1]]);
  ok('в обложке только модуль', D.обложкаТекст === ГАЙДЫ[0][1].split('—')[0].trim(),
      [D.обложкаТекст, ГАЙДЫ[0][1]]);
  ...
  ok('полное имя категории осталось в подсказке',
      !!D.длиннаяКатегория && (D.подсказкаДобавить || '').indexOf(D.длиннаяКатегория) >= 0,
      [D.подсказкаДобавить, D.длиннаяКатегория]);
