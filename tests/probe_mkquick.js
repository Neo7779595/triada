...
(async () => {
  ...
  const A = await page.evaluate(() => {
    ...
    return {
      ...
      верх: h.querySelector('.mk-bar').textContent.replace(/\s+/g, ' ').trim(),
  ...
  ok('кнопок групп на одну больше, чем групп: первая — «Все»',
    A.чипов === A.групп + 1, A);
  /* Шапку модуля владелец сознательно оставил пустой: только валюта и курс.
     Новый экран не имеет права вернуть туда ничего своего. */
  ok('в шапке модуля по-прежнему только валюта и курс', A.верх === 'сум $ курс', A.верх);
