...
(async () => {
  ...
  /* Наведение поднимает карточку и подсвечивает рамку. Цветного свечения
     из-под карточки быть не должно: оно читалось как подсветка кнопки в
     игровом лаунчере и спорило с бирюзовым ответом внутри — глаз шёл на
     пятно, а не на число. */
  const CSS = require('fs').readFileSync(__dirname + '/../index.html', 'utf8');
  const правила = (CSS.match(/\n\.mk-qc(:hover|::after|::before)?\{[^}]*\}/g) || []);
  const свечение = правила.filter(t => /55, ?230, ?200|--accent|radial-gradient/.test(t));
  ok('под карточкой нет цветного свечения — ни на наведении, ни фоном',
    свечение.length === 0, свечение.map(t => t.slice(0, 120)));
  ok('и правило наведения на месте, а не потерялось целиком',
    правила.some(t => /:hover/.test(t) && /translateY/.test(t)), правила.length);
  ...
  /* Раз валюта стоит в одном ряду с фильтрами — она и выглядеть должна так же.
     Прямоугольник посреди ряда пилюль читается как чужой элемент. */
  const ФОРМА = await page.evaluate(() => {
    const r = e => getComputedStyle(e).borderRadius;
    const h = e => Math.round(e.getBoundingClientRect().height);
    const чип = document.querySelector('.mk-qchip');
    const вал = document.querySelector('.mk-qchips .mk-cur button');
    const кур = document.querySelector('.mk-qchips .mk-rate input');
    const шапка = document.querySelector('.mk-qhead');
    const карта = document.querySelector('.mk-qgrid .mk-qc');
    return { чипR: r(чип), валR: r(вал), курR: r(кур),
      чипH: h(чип), валH: h(вал), курH: h(кур),
      шапкаH: h(шапка),
      зазор: Math.round(карта.getBoundingClientRect().top - шапка.getBoundingClientRect().bottom) };
  });
  ok('валюта и курс — такие же овалы, как фильтры рядом',
    ФОРМА.валR === ФОРМА.чипR && ФОРМА.курR === ФОРМА.чипR, ФОРМА);
  ok('и того же роста', ФОРМА.валH === ФОРМА.чипH && ФОРМА.курH === ФОРМА.чипH, ФОРМА);
  /* Шапка обязана оставаться полосой, а не блоком в треть экрана; при этом
     карточки не должны в неё упираться. Одно за счёт другого не берём. */
  ok('шапка занимает полосу, а не треть экрана', ФОРМА.шапкаH <= 130, ФОРМА.шапкаH);
  ok('между шапкой и первой карточкой есть воздух', ФОРМА.зазор >= 24, ФОРМА.зазор);
