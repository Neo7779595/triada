...
(async () => {
  ...
  /* Число сюда не вписываем: список метрик растёт, и проверка, знающая
     своё «шестнадцать», начнёт падать на каждой новой карточке. Смысл в
     том, что описана каждая, а не в том, сколько их сегодня. */
  ok('перечислены все быстрые расчёты — все ' + INV.quick.length,
    mQ.length === 0 && INV.quick.length >= 16, mQ);
