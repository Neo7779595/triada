/* probe_finres — резерв: правило пополнения и запас в месяцах.
   ...
   База — остаток августа, тот же, что в шаге «Распределение»:
     приход 10 000 000 + 5 000 000 = 15 000 000
     расход  2 000 000 +   500 000 =  2 500 000   (второй — с резервной карты)
     остаток                          12 500 000
   В резерв следовало 1 250 000, переведено 1 000 000 → не отложено 250 000.
   Проценты нарочно берутся с остатка, а не с поступлений: в месяц, где
   потратили всё, второе правило требовало бы отложить с денег, которых
   уже нет.
...
(async () => {
  ...
  ok('база доли — остаток 12 500 000, а не поступления', base.base === 12500000, base.base);
  ok('по доле 10% в резерв следовало 1 250 000', base.shouldSave === 1250000, base.shouldSave);
  ok('переведено 1 000 000', base.saved === 1000000, base.saved);
  ok('долг перед собственным резервом 250 000', base.owe === 250000, base.owe);
