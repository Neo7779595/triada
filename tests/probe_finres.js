/* probe_finres — резерв: правило пополнения и запас в месяцах.
   ...
   Доля резерва в распределении — 10% остатка, цель — 3 месяца расходов.

   ── На бумаге ───────────────────────────────────────────────────────
   Резерв = 2 000 000 + 1 000 000 − 500 000 = 2 500 000.
   База — та же, что в шаге «Распределение»: остаток месяца за вычетом
   авансов.
     приход 10 000 000 + аванс 5 000 000 = 15 000 000
     расход  2 000 000 +      500 000    =  2 500 000  (второй — с резерва)
     остаток                                12 500 000
     минус аванс 5 000 000                   7 500 000  ← база
   В резерв следовало 750 000, переведено 1 000 000 → долга нет.
   Проценты берутся с остатка, а не с поступлений: в месяц, где потратили
   всё, второе правило требовало бы отложить с денег, которых уже нет.
   Аванс из базы вычтен нарочно: это деньги за работу, которой ещё не было,
   и откладывать с них процент значит откладывать с чужого.
...
(async () => {
  ...
  ok('поступления августа — 15 000 000', base.periodIn === 15000000, base.periodIn);
  /* База — не поступления, а остаток: приход минус расход. Это то же число,
     что и в шаге «Распределение», и оно там одно на все четыре доли. */
  ok('расход августа 2 500 000, включая трату с самой резервной карты',
    base.periodOut === 2500000, base.periodOut);
  ok('остаток месяца 12 500 000, но база доли — 7 500 000: аванс не делится',
    base.base === 7500000, base.base);
  ok('по доле 10% в резерв следовало 750 000', base.shouldSave === 750000, base.shouldSave);
  ok('переведено 1 000 000', base.saved === 1000000, base.saved);
  ok('переведено больше, чем требовала доля, — долга нет', base.owe === 0, base.owe);
